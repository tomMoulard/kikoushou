...
export interface UseTransportScopeResult<TTransport extends Transport> {
   ...
   *
   * Exposed so a view can scope things that are not rows — the map's
   * rendez-vous markers, the list's ride cards — against the *same* identity
   * this hook filtered with. A second `useTripIdentity()` at the call site
   * would resolve independently and could disagree with it for a frame, which
   * is how a car ends up on the map with its passengers filtered out from
   * under it.
   */
